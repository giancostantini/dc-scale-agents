import { clients, representatives, products, months, seasonality } from '../content/mundipack-demo.js';

const money = value => new Intl.NumberFormat('es-UY', { style: 'currency', currency: 'UYU', maximumFractionDigits: 0 }).format(value);
const compact = value => `$${(value / 1000).toLocaleString('es-UY', { maximumFractionDigits: value < 10000 ? 1 : 0 })}k`;
...
// Historial de 6 meses por cliente: Abr–Jul salen de "mes anterior" con una
// estacionalidad y una variación fija por cliente; Ago = previous; Sep = current.
function history(c) {
  const base = c.previous || c.current;
  const wobble = [...c.id].reduce((acc, ch) => acc + ch.charCodeAt(0), 0) % 7 / 100;
  return [...seasonality.map((f, i) => (c.previous ? Math.round(base * (f + (i % 2 ? wobble : -wobble)) / 100) * 100 : 0)), c.previous, c.current];
}
...
export function createMundipackDemo() {
  ...
  const states = {
    director: { page: 'overview', period: 'current', rep: 'all', segment: 'Todos', query: '', family: 'Todas', client: null },
    seller: { page: 'today', period: 'current', rep: 'valentina', segment: 'Todos', query: '', family: 'Todas', client: null, started: false, cart: {}, discount: 0, quoteClient: 'Café Azahar' },
  };
  // Las visitas son UNA sola fuente: lo que marca el vendedor lo ve Dirección en Rutas.
  const visits = new Set(['trigal', 'bruma']);
  const seller = () => representatives.find(r => r.id === states.seller.rep);
  let role = 'director';
  let chatOpen = false;
  ...
  const customerButton = c => `<button class="mp-customer" type="button" data-action="client" data-value="${c.id}"><span class="mp-avatar">${c.initials}</span><span><strong>${c.name}</strong><small>${c.zone} · ${c.days} días desde su última compra · ${c.orders} ${c.orders === 1 ? 'pedido' : 'pedidos'} este mes</small></span><span aria-hidden="true">↗</span></button>`;

  root.innerHTML = `<div class="mp-demo-toolbar"><div class="mp-role-switch" role="group" aria-label="Elegir dashboard de Mundipack">${button('Dirección / Gerencia', 'role', 'director', 'aria-pressed="true"')}${button('Vendedores', 'role', 'seller', 'aria-pressed="false"')}</div><label class="mp-as" data-mp-as hidden>Ver como<select data-field="seller">${representatives.map(r => `<option value="${r.id}">${r.name}</option>`).join('')}</select></label><span class="mp-demo-label"><span aria-hidden="true">●</span> Demo interactiva · datos ficticios</span></div><p class="mp-guide">Explorá el negocio, abrí un cliente y cambiá de dashboard para vivir el día del vendedor.</p><div class="mp-window"><div class="mp-window-bar"><span class="mp-window-dots" aria-hidden="true">● ● ●</span><span>Mundipack / <span data-mp-role-name>Dirección</span></span><span class="mp-window-badge">ENTORNO DEMO</span></div><div class="mp-app"><aside class="mp-sidebar"><div class="mp-brand"><span>${icon('box')}</span><div>Mundipack<small data-mp-subtitle>GESTIÓN COMERCIAL</small></div></div><div class="mp-sidebar-label">TU ESPACIO DE TRABAJO</div><nav class="mp-nav" aria-label="Secciones del dashboard"></nav><div class="mp-session"><span class="mp-avatar" data-mp-avatar>DG</span><div data-mp-session>Dirección<small>Perfil de demostración</small></div></div></aside><div class="mp-viewport" tabindex="0" role="region" aria-label="Contenido interactivo de Mundipack"><div class="mp-view"></div></div></div><div class="mp-assistant" hidden></div><button type="button" class="mp-assistant-toggle" data-action="assistant" aria-expanded="false">${icon('spark')}<span>Asistente demo</span></button></div><p class="mp-feedback" role="status" aria-live="polite">Elegí una sección del menú para comenzar.</p>`;
  ...
  function chart(rows = portfolio()) {
    const series = months.map((_, i) => rows.reduce((total, c) => total + history(c)[i], 0));
    const max = Math.max(...series, 1);
    const highlight = state().period === 'current' ? 5 : 4;
    return `<div class="mp-card"><div class="mp-card-title"><h5>Evolución de ventas</h5><span>UYU · datos de ejemplo</span></div><div class="mp-chart" role="img" aria-label="Ventas ficticias de abril a septiembre: ${series.map((v, i) => `${months[i]} ${money(v)}`).join(', ')}">${series.map((v, i) => `<div class="mp-chart-column"><span>${compact(v)}</span><div class="mp-chart-bar ${i === highlight ? 'is-current' : ''}" style="--bar:${(v / max).toFixed(3)};--i:${i}"></div><small>${months[i]}</small></div>`).join('')}</div></div>`;
  }
  function dashboard() {
    const amount = sales();
  ...
  function clientDetail() {
    ...
    return `${button('← Volver a la cartera', 'back')} ${title('FICHA DE CLIENTE · DEMO', c.name, `${c.zone} · Responsable: ${repName(c.rep)}`)}<div class="mp-stats">${metric('Ventas del mes', money(c.current), `${c.orders} ${c.orders === 1 ? 'pedido' : 'pedidos'} este mes`)}${metric('Última compra', `Hace ${c.days} días`, c.segment)}${metric('Saldo pendiente', money(c.debt), c.debt ? 'Seguimiento de cobranza' : 'Al día')}</div>${chart([c])}<div class="mp-two-col"><div class="mp-card"><h5>El contexto antes de visitar</h5><p>${c.segment === 'En riesgo' ? 'La frecuencia de compra bajó. Una visita puede ayudar a entender qué cambió y recuperar el vínculo.' : 'La relación está activa. Revisá sus necesidades para preparar la próxima visita.'}</p><div class="mp-timeline"><div><span>Última compra</span><strong>${products.find(p => p.id === c.last)?.name}</strong><small>Registro de ejemplo · ${c.days} días atrás</small></div><div><span>Seguimiento comercial</span><strong>Revisar necesidades de reposición</strong><small>Nota de demostración</small></div></div></div><div class="mp-card"><h5>Próximo paso</h5><p>Convertí la información del cliente en una acción concreta.</p>${role === 'seller' ? `${button('Preparar presupuesto ↗', 'client-quote', c.id)}${button('Ver mi ruta', 'navigate', 'route')}` : button('Ver cobranzas ↗', 'navigate', 'collections')}${button('Consultar al asistente ✧', 'assistant-client', c.id)}</div></div>`;
  }
  ...
  function productResults() {
    ...
    return `<div class="mp-table-wrap"><table class="mp-table"><caption>${rows.length} productos de demostración · precios en UYU</caption><thead><tr><th>Producto</th><th>Familia</th><th>Precio</th><th>${role === 'director' ? 'Stock' : 'Presupuesto'}</th></tr></thead><tbody>${rows.map(p => `<tr><td><strong>${p.name}</strong><small>${p.code}</small></td><td>${p.family}</td><td>${money(p.price)}${role === 'seller' ? `<small class="${p.stock < 30 ? 'mp-low' : ''}">${p.stock < 30 ? 'Poco stock · ' : ''}${p.stock} packs</small>` : ''}</td><td>${role === 'director' ? `<span class="${p.stock < 30 ? 'mp-low' : ''}">${p.stock} packs${p.stock < 30 ? ' · reponer' : ''}</span>` : button('Agregar +', 'add-product', p.id, `aria-label="Agregar ${p.name} al presupuesto"`)}</td></tr>`).join('')}</tbody></table>${!rows.length ? '<p class="mp-empty">No hay productos con esos filtros.</p>' : ''}</div>`;
  }
  ...
  function map(rows) {
    const line = rows.map(c => `${c.pos[0] * 5},${c.pos[1] * 2.2}`).join(' ');
    return `<div class="mp-map" role="group" aria-label="Esquema ilustrativo de la ruta, sin ubicaciones reales"><svg viewBox="0 0 500 220" preserveAspectRatio="none" aria-hidden="true"><path class="mp-map-water" d="M0 160Q120 100 160 190T500 200V220H0Z"/><path class="mp-map-streets" d="M0 35h500M0 80h500M0 125h500M0 170h500M50 0v220M125 0v220M200 0v220M275 0v220M350 0v220M425 0v220"/>${rows.length > 1 ? `<polyline class="mp-map-route" points="${line}" fill="none"/>` : ''}</svg><span class="mp-map-caption">Recorrido ilustrativo</span>${rows.map((c, i) => `<button type="button" class="mp-map-pin ${visits.has(c.id) ? 'is-done' : ''}" style="left:${c.pos[0]}%;top:${c.pos[1]}%" data-action="client" data-value="${c.id}" aria-label="${visits.has(c.id) ? 'Visitado: ' : ''}abrir ficha de ${c.name}">${visits.has(c.id) ? '✓' : i + 1}</button>`).join('')}</div>`;
  }
  function route(director = false) {
    const rows = portfolio();
    const done = rows.filter(c => visits.has(c.id)).length;
    const who = director ? repName(state().rep) : 'Ruta de hoy';
    const perRep = director && state().rep === 'all' ? `<div class="mp-stats">${representatives.map(rep => { const own = clients.filter(c => c.rep === rep.id); const ok = own.filter(c => visits.has(c.id)).length; return `<div class="mp-card"><span class="mp-avatar">${rep.initials}</span><h5>${rep.name}</h5><strong class="mp-number">${ok} / ${own.length}</strong><small>visitas de hoy</small>${progress(ok / own.length * 100, `Visitas de ${rep.name}`)}</div>`; }).join('')}</div>` : '';
    return `${title(director ? 'COORDINACIÓN DEL EQUIPO' : 'TU DÍA', director ? 'Rutas del equipo, en vivo.' : 'Tu próxima visita, con contexto.', director ? 'Lo que marca cada vendedor en su portal aparece acá al instante. Probá marcar una visita desde Vendedores.' : 'Abrí una parada o marcá una visita: Dirección la ve al instante.')}${director ? filters() : ''}<div class="mp-card"><div class="mp-card-title"><h5>${who}</h5><strong>${done} / ${rows.length} visitas</strong></div>${progress(done / rows.length * 100, 'Visitas completadas')}</div>${perRep}${map(rows)}<div class="mp-card mp-route-list">${rows.map((c, i) => `<div class="mp-route-stop"><span class="mp-stop-index ${visits.has(c.id) ? 'is-done' : ''}">${visits.has(c.id) ? '✓' : i + 1}</span>${customerButton(c)}${director ? `<span class="mp-tag ${visits.has(c.id) ? '' : 'mp-tag-warm'}">${visits.has(c.id) ? 'Hecha' : 'Pendiente'}${state().rep === 'all' ? ` · ${repName(c.rep)}` : ''}</span>` : button(visits.has(c.id) ? 'Visita hecha ✓' : 'Marcar visita', 'visit', c.id, `aria-pressed="${visits.has(c.id)}" aria-label="${visits.has(c.id) ? 'Desmarcar' : 'Marcar'} visita a ${c.name}"`)}</div>`).join('')}</div>`;
  }
  function today() {
    const done = portfolio().filter(c => visits.has(c.id)).length;
    return `${title('PORTAL DEL VENDEDOR', `Buen día, ${seller().name}.`, 'Tu cartera, tus visitas y tus oportunidades.')}<div class="mp-day"><div><strong>${state().started ? 'Jornada iniciada' : 'Tu jornada'}</strong><small>${state().started ? 'Ya podés registrar las visitas de hoy.' : 'Prepará tu recorrido y empezá el día.'}</small></div>${button(state().started ? 'Finalizar jornada' : 'Iniciar jornada', 'day')}</div><div class="mp-two-col"><div class="mp-summary-primary"><span class="mp-eyebrow">TU OBJETIVO DEL MES</span><strong>${money(sales())}</strong><p>de ${money(target())}</p>${progress(sales() / target() * 100, 'Objetivo del vendedor')}</div><div class="mp-card"><div class="mp-card-title"><h5>Tu ruta de hoy</h5>${button('Ver ruta ↗', 'navigate', 'route')}</div><strong class="mp-number">${done} / ${portfolio().length} visitas</strong>${progress(done / portfolio().length * 100, 'Tu ruta de hoy')}<small>Próxima parada: ${portfolio().find(c => !visits.has(c.id))?.name || 'Recorrido completado'}</small></div></div><div class="mp-two-col"><div class="mp-card"><h5>Clientes que necesitan atención</h5>${portfolio().filter(c => c.debt || c.segment === 'En riesgo').map(customerButton).join('')}</div>${map(portfolio())}</div>`;
  }
  ...
  function render(focus = false) {
    ...
    root.querySelector('[data-mp-avatar]').textContent = role === 'director' ? 'DG' : seller().initials;
    root.querySelector('[data-mp-session]').innerHTML = `${role === 'director' ? 'Dirección' : seller().name}<small>Perfil de demostración</small>`;
    const as = root.querySelector('[data-mp-as]');
    as.hidden = role !== 'seller';
    as.querySelector('select').value = states.seller.rep;
    ...
    const pages = { overview: dashboard, team, clients: () => s.client ? clientDetail() : clientList(), products: productList, collections, routes: () => route(true), today, route, opportunities, quote, progress: progressPage };
    view.innerHTML = pages[s.page]();
    if (focus) { viewport.scrollTop = 0; view.querySelector('h4')?.focus({ preventScroll: true }); }
  }
  function progressPage() {
    const weeks = [0.22, 0.48, 0.71, 1].map(f => Math.round(sales() * f));
    const goal = target();
    const own = portfolio().filter(c => visits.has(c.id)).length;
    return `${title('TU EVOLUCIÓN', 'Cada paso cuenta.', 'Cómo venís contra tu objetivo, semana a semana.')}${filters()}<div class="mp-card"><div class="mp-card-title"><h5>Avance acumulado del mes</h5><span>Objetivo ${money(goal)}</span></div><div class="mp-weeks">${weeks.map((v, i) => `<div class="mp-week"><span>Semana ${i + 1}</span>${progress(v / goal * 100, `Semana ${i + 1}`)}<strong>${money(v)}</strong></div>`).join('')}</div><small>${sales() >= goal ? 'Objetivo alcanzado 🎯' : `Te faltan ${money(goal - sales())} para el objetivo.`}</small></div>${chart()}<div class="mp-card"><h5>Cumplimiento de visitas</h5>${progress(own / portfolio().length * 100, 'Cumplimiento de visitas')}<p>${own} de ${portfolio().length} visitas realizadas en tu ruta.</p>${button('Continuar mi ruta ↗', 'navigate', 'route')}</div>`;
  }
  ...
  root.addEventListener('click', event => {
    ...
    if (action === 'visit') { if (visits.has(value)) visits.delete(value); else visits.add(value); render(); root.querySelector(`[data-action="visit"][data-value="${value}"]`)?.focus({ preventScroll: true }); say(`${clients.find(c => c.id === value).name}: visita ${visits.has(value) ? 'completada' : 'pendiente'}. Dirección ya lo ve en Rutas.`); }
  ...
  root.addEventListener('change', event => {
    const field = event.target.dataset.field;
    if (field === 'seller') { states.seller.rep = event.target.value; states.seller.client = null; render(); root.querySelector('[data-field="seller"]')?.focus({ preventScroll: true }); say(`Portal de ${seller().name}.`); }
