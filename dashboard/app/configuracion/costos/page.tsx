...
/**
 * /configuracion/costos — Panel de gasto de la API de Claude. Solo director.
 * Lee /api/usage (agrega api_usage + calcula costo). Muestra total del período
 * + desglose por agente/endpoint, por cliente y por modelo.
 *
 * Nota: solo refleja lo que ya está instrumentado (los call sites que registran
 * usage). La fuente de verdad absoluta del gasto es console.anthropic.com →
 * Usage; este panel es para ver el desglose POR agente/cliente que la consola
 * no da.
 */
  ...
  return (
    <>
      ...
      <main style={wrap}>
        <div style={head}>
          <div>
            <div style={eyebrow}>Configuración · Costos API</div>
            <h1 style={h1}>Gasto de la API de Claude</h1>
            <div style={sub}>
              Últimos {days} días. La fuente de verdad total es{" "}
              <a
              ...
              ; acá ves el desglose por agente y por cliente.
            </div>
          </div>
          <select
            value={days}
        ...
        {!loading && data && (
          <>
            ...
            {data.calls === 0 ? (
              <div style={panel}>
                Todavía no hay registros de uso en este período. Empieza a
                poblarse cuando corran los agentes / se use el dashboard (con el
                logging ya activo).
              </div>
            ) : (
              <div style={grid}>
...
function Stat({ label, value, big }: { label: string; value: string; big?: boolean }) {
  return (
    <div style={statCard}>
      ...
      <div style={{ ...statValue, fontSize: big ? 30 : 20 }}>{value}</div>
    </div>
  );
}
        ...
        {rows.map((r) => (
          <div key={r.key} style={tableRow}>
            <div style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
              {r.key}
              <span style={{ color: "var(--text-muted)", fontSize: 11 }}> · {r.calls}</span>
            </div>
