...
export interface DividendDistribution {
  ...
  currency: FinanceCurrency;
  /** Secuencia dentro del mes/moneda (migración 099). 0 = distribución
   *  original; >=1 = reajustes que reparten solo la diferencia. */
  seq: number;
  net_profit: number;
  partner_a_pct: number;
  partner_b_pct: number;
...
export async function setDividendDistributionStatus(
  ...
  // creado".
  await syncDividendDistributionMovement(monthKey, currency, seq);
}

/** Marker del movimiento bancario del dividendo. Para seq=0 mantiene el
 *  formato histórico (sin sufijo) para NO duplicar los movimientos ya
 *  creados antes de la migración 099; los reajustes (seq>=1) llevan el
 *  seq en el marker para tener su propio movimiento. */
const DIVIDEND_MOVEMENT_MARKER = (
  monthKey: string,
  currency: FinanceCurrency,
  seq = 0,
) =>
  seq === 0
    ...
    : `[auto-dividend:${monthKey}:${currency}:${seq}]`;
...
async function syncDividendDistributionMovement(
  ...
  const supabase = getSupabase();
  const marker = DIVIDEND_MOVEMENT_MARKER(monthKey, currency, seq);

  // Leer el snapshot actual (de esta moneda + seq)
  const { data: snap } = await supabase
    .from("dividend_distributions")
    .select(
      "month_key, seq, status, cuenta_id, partner_a_amount, partner_b_amount, notes",
    )
    .eq("month_key", monthKey)
    .eq("currency", currency)
    .eq("seq", seq)
  ...
  const description =
    seq === 0
      ? `Distribución dividendos ${currency} · ${monthLabel}`
      : `Reajuste dividendos ${currency} · ${monthLabel}`;
...
/**
 * Crea un REAJUSTE de la distribución de un mes/moneda: una fila nueva
 * (seq>=1) que reparte SOLO `deltaNet` (la diferencia detectada después
 * de pagar la original), sin tocar la original (seq=0). Nace en estado
 * pending; el director la paga como cualquier otra distribución.
 *
 * El label ("Reajuste distribución de <mes>") se guarda en notes para
 * mostrarlo en el historial.
 */
export async function createDividendReajuste(
  monthKey: string,
  currency: FinanceCurrency,
  deltaNet: number,
  config: DividendConfig,
